...
export default async function ApiKeysPage() {
  ...
  if (!session) redirect('/');
  if (!session.user.hasGithubApp) redirect('/connect-github');
  ...
  return (
    <div className='flex min-h-screen flex-col items-center justify-center gap-10 px-4 py-16'>
      <div className='flex flex-col items-center gap-3 text-center'>
        ...
        <h1 className='text-3xl font-bold tracking-tight'>Set up your AI provider</h1>
        <p className='max-w-md text-muted-foreground'>
          MergeLens uses AI to review your pull requests. Add your own API key to use your
          preferred provider and control your own billing.
        </p>
      </div>

      <div className='grid w-full max-w-4xl grid-cols-1 gap-6 lg:grid-cols-2'>
        <ApiKeysCard configuredProviders={configuredProviders} />
        <ProviderCard configuredProviders={configuredProviders} current={preferredProvider} />
      </div>
